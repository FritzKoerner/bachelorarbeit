...
Ported verbatim from prototyp_2/controllers/low_level_controller.py for use in
the global coordinate landing project (prototyp_global_coordinate).
"""
...
class VelocityController:
    ...
    def compute_rpm(
        ...
        # Transform target and current velocity into body frame for correct roll/pitch mapping
        target_vel_body = self._world_to_body(target_vel, current_quat)
        current_vel_body = self._world_to_body(current_vel, current_quat)

        # Velocity PD control (in body frame)
        vel_error = target_vel_body - current_vel_body
    ...
    def _world_to_body(self, vec: torch.Tensor, quat: torch.Tensor) -> torch.Tensor:
        """Rotate a world-frame vector into body frame using the inverse quaternion."""
        w, x, y, z = quat[:, 0], quat[:, 1], quat[:, 2], quat[:, 3]
        # Inverse quaternion (conjugate for unit quat)
        inv_w, inv_x, inv_y, inv_z = w, -x, -y, -z
        # Rotate vec by inverse quat: q_inv * vec * q
        vx, vy, vz = vec[:, 0], vec[:, 1], vec[:, 2]
        # t = 2 * cross(q_xyz, v)
        tx = 2 * (inv_y * vz - inv_z * vy)
        ty = 2 * (inv_z * vx - inv_x * vz)
        tz = 2 * (inv_x * vy - inv_y * vx)
        # result = v + w * t + cross(q_xyz, t)
        rx = vx + inv_w * tx + (inv_y * tz - inv_z * ty)
        ry = vy + inv_w * ty + (inv_z * tx - inv_x * tz)
        rz = vz + inv_w * tz + (inv_x * ty - inv_y * tx)
