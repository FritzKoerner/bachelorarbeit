...
2. **SSH into the cluster and run setup:**
   ...
   ```
   This creates the `ba` conda env with PyTorch (cu126), Genesis, rsl-rl, W&B, etc.
...
./hpc/run_interactive.sh              # default: paula, a30, 1h
./hpc/run_interactive.sh clara v100 2 # clara partition, v100, 2h

# Once allocated:
source ~/genesis/hpc/setup_env.sh --load
cd ~/genesis/prototyp_global_coordinate
...
```bash
ssh hpc
sbatch ~/genesis/hpc/jobs/train_global_coord.job
sbatch ~/genesis/hpc/jobs/train_obstacle_avoidance.job
```

**Option B: Submission wrapper** (configure from command line):
```bash
# Global coordinate — full training on paula
./hpc/submit_training.sh global_coordinate --batch 4096 --iters 401

# Obstacle avoidance — on clara with V100
./hpc/submit_training.sh obstacle_avoidance --partition clara --gpu-type v100 --batch 512
...
# Then on HPC:
source ~/genesis/hpc/setup_env.sh --load
tensorboard --logdir ~/genesis/prototyp_global_coordinate/logs --port 6006
